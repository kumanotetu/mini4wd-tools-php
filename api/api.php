...
if(empty($message)) {
    ...
    $j_param = [
        'result-parameter' => [
            'f-height-out' => round($f_height_out * 100) / 100,
            'f-height-in'  => round($f_height_in  * 100) / 100,
            'r-height-out' => round($r_height_out * 100) / 100,
            'r-height-in'  => round($r_height_in  * 100) / 100,
            'f-angle' => round($f_angle * 10) / 10,
            'r-angle' => round($r_angle * 10) / 10
        ]
    ];
    ...
    return [
        ...
        'description' => [
            ...
            'input-parameter' => [
                'wh-base'  => 'ホイールベース(mm)',
                't-size'   => 'タイヤ直径(mm)',
                'f-length' => '前車軸からフロントブレーキ内側までの距離(mm)',
                'f-width'  => 'フロントブレーキ内側から外側までの距離、ブレーキの幅(mm)',
                'r-length' => '後車軸からリアブレーキ内側までの距離(mm)',
                'r-width'  => 'リアブレーキ内側から外側までの距離、ブレーキの幅(mm)',
                'check-bank' => [
                    '1' => 'JCJCバンク',
                    '6' => 'ライジングファントムチェンジャー'
                ]
            ...
            'result-parameter' => [
                'f-height-out' => 'フロントブレーキ外側の高さ(mm)',
                'f-height-in'  => 'フロントブレーキ内側の高さ(mm)',
                'r-height-out' => 'リアブレーキ外側の高さ(mm)',
                'r-height-in'  => 'リアブレーキ内側の高さ(mm)',
                'f-angle' => 'フロントブレーキの角度(°)',
                'r-angle' => 'リアブレーキの角度(°)',
            ]
        ],
